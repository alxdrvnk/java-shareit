...
public class BookingProviderPastState implements BookingProvider {
    ...
    public Collection<Booking> getBookingsOfUser(Long userId, String state, int from, int size) {
        ...
        return bookingRepository
                .findAllByBookerIdAndEndBeforeOrderByStartDesc(userId, LocalDateTime.now(clock), pageable);
    }

    @Override
    public Collection<Booking> getBookingsOfOwnerItems(Long userId, String state, int from, int size) {
        Pageable pageable = PageRequest.of(from / size, size);
        return bookingRepository.findAllByOwnerIdAndPastState(userId, LocalDateTime.now(clock), pageable);
    }
}
